...
for Nw in Nws:
     ...
     for ik1 in range(Nk):
         for ik2 in range(Nk):
             for n in range(Nw):
                 x0 += 1./(beta*Nk**2) * 1./(Z[ik1,ik2,n]**2*wn[n]**2 + band[ik1,ik2]**2)
     ...
     if abs(g) > 1e-10:  # g is nonzero         
         ...
         #t = zeros([Nk,Nk,Nw,Nw], dtype=complex)
         #t = einsum('ij,kl->ijkl', gofq2, D)                      
         t = g2D.copy();
         change = 1.0
         ...
         for myiter in range(iter_selfconsistency):
             ...
             for iq1 in range(Nk):
                 for iq2 in range(Nk):
                     if (iq1+iq2*Nk)%nprocs == myrank:
                         for ik1 in range(Nk):
                             for ik2 in range(Nk):
                                 for n in range(Nw):
                                     for np in range(Nw):
                                         for npp in range(Nw):
                                             tnew_proc[iq1,iq2,np,n] -= 1./(beta*Nk**2)*1./(Z[ik1,ik2,npp]**2*wn[npp]**2 + band[ik1,ik2]**2)*g2D[iq1,iq2,npp,np] * t[iq1,iq2,npp,n]
         ...
         for ik1 in range(Nk):
             for ik2 in range(Nk):
                 if (ik1+ik2)%nprocs == myrank:
                     for ip1 in range(Nk):
                         for ip2 in range(Nk):
                             iqx, iqy = subtract_momenta(ip1,ip2,ik1,ik2,Nk)
                             for n in range(Nw):
                                 for np in range(Nw):
                                     x_proc -= real( 1./(beta*Nk**2)**2 * 1./(Z[ip1,ip2,np]**2*wn[np]**2 + band[ip1,ip2]**2) * t[iqx,iqy,np, n] * 1./(Z[ik1,ik2,n]**2*wn[n]**2 + band[ik1,ik2]**2) )
